...
import { AddVaccinationCardData, LoadVaccinationCardData, LoadVaccinationCardDataFail, UpdateVaccinationCardDataProgress } from '../actions';
...
export class ReportDataEffects {
    ...
    this.actions$.pipe(
      ...
      switchMap((actions) =>
        this.getVaccinationCardDataFromServer(actions).pipe(
          map((vaccinationCardData: Array<VaccinationCard>) => AddVaccinationCardData({ vaccinationCardData })),
  ...
  getVaccinationCardDataFromServer(parameters: any) {
    const { vaccinationCardConfigs , selectedPeriods ,selectedOrgUnits } = parameters;
